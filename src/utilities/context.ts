...
const { Identifier, BlockStatement, Literal, UnaryExpression } = types.namedTypes;
...
function assignify(node: Node, ctx: string) {
	if (isReplaceable(node)) {
		if (types.namedTypes.AssignmentExpression.check(node)) {
...
function visitExpressions(ast: RecastAST, ctx: string, globals: string[]) {
	types.visit(ast, {
		...
		visitSequenceExpression(path) {
			let { node } = path;
			node.expressions.forEach((exp, i) => {
				canBeReplaced(exp, path, globals) && replace('expressions', ctx, path, i);
			});
			return this.traverse(path);
		},
		...
		visitAssignmentExpression(path) {
			...
			if (canBeReplaced('argument' in node ? node['argument'] : node.left, path, globals)) {
				let newNode = assignify(node, ctx);
	...
	while (iterations < 5) {
		types.visit(ast, {
			...
			visitIfStatement(path) {
				...
				} else if (types.namedTypes.IfStatement.check(node.alternate)) {
					this.traverse(path);
				}
...
function isReplaceable(node: Node) {
	return Identifier.check(node) ?
		true : types.namedTypes.AssignmentExpression.check(node) ?
			isReplaceable((<any>node).left) : types.namedTypes.UpdateExpression.check(node) ?
				isReplaceable((<any>node).argument) : isReplaceable((<any>node).object);
}
