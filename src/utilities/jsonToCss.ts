import { hash, camelToKebabCase } from './tools';
...
function abstractStyle(options: { [key: string]: any }) {
	const localPlugins: { [key: string]: any } = {};
	let HASHCLASS = '____HASHCLASS____';

	localPlugins['keyframes'] = keyframes;

	let abtStyle: any = (objRules: Object, scope?: string) => {
		let scopeClass = scope ? `scope_${hash(scope)}` : HASHCLASS;
		let ruleResgitered = [];
		...
		return { className, styleText: minify(styleText) };
	};
