...
interface AbtStyle {
  (objRules: Object, scope?: string): { className: string, styleText: string };
  [key: string]: any;
}
...
function toCSS(rules: { [key: string]: any }, options: { [key: string]: any }, indent?: string[]) {
  let css = '';
  ...
  for (let selector in rules) {
    ...
    if (~selector.indexOf('____raw')) {
      ...
      for (let prop in rules[selector]) {
        ...
        prop = prop.replace(/[$]\d*/, '');
        prop = options && options.keepCamelCase === true ? prop : prop.replace(/([A-Z])/g, w => `-${w.toLowerCase()}`);
...
// combining selectors
function combineSelectors(rules: { [key: string]: any }, preventCombining: string[], keepCamelCase: boolean) {
  let map = [], arr = {};
  ...
  for (let i = 0; i < map.length; i++) {
    if (map[i].combine === true && map[i].selector !== false) {
      for (let j = i + 1; j < map.length; j++) {
        let propi = keepCamelCase ? map[i].prop : camelToSnake(map[i].prop);
        let propj = keepCamelCase ? map[j].prop : camelToSnake(map[j].prop);
...
function processor(rules: { [stylesheet: string]: { [key: string]: any } }, options?: { [key: string]: any }) {
...
function abstractStyle(options: { minify?: boolean }) {
  const localPlugins: { [key: string]: any } = {};
  let HASHCLASS = '____HASHCLASS____';

  localPlugins['keyframes'] = keyframes;

  let abtStyle: AbtStyle = (objRules: Object) => {
    let ruleResgitered = [];
    let allRules = { mainstream: {} };
    const opts = { ...{ combineSelectors: true, preventCombining: ['@font-face'] }, ...(options || {}) };
    ...
    function addRule(selector, props, stylesheet, parentSelector?) {
      ...
      // multiple selectors
      if (/, ?/g.test(selector) && opts.combineSelectors) {
      ...
      for (let prop in _objects) {
        ...
        if (prop.charAt(0) === ':') {
          ...
          if (/, ?/g.test(prop) && opts.combineSelectors) {
        ...
        } /* else if (prop.indexOf('@media') === 0 || prop.indexOf('@supports') === 0 || prop.indexOf('@document') === 0) {
					addRule(selector, _objects[prop], prop, parentSelector);
					// check for media query
				} */ else if (selector.indexOf('@media') === 0 || prop.indexOf('@supports') === 0 || prop.indexOf('@document') === 0) {
    ...
    let styleText = processor(allRules, opts);
    ...
    return { className, styleText: opts.minify ? minify(styleText) : styleText };
  };

  abtStyle.minify = minify;

  abtStyle.prop = (pluginName: string, pluginAction: (value: any) => { [key: string]: any }) => {
    localPlugins[pluginName] = pluginAction;
    return abtStyle;
  };

  abtStyle.process = (rules: { [key: string]: any }, options: { [key: string]: any }) => {
    return processor({ mainstream: rules }, options);
  };

  abtStyle.raw = (rules: { [key: string]: any }, options: { [key: string]: any }) => {
...
function keyframes(value: { [key: string]: any }) {
  if (typeof value === 'object') {
    let frames;
    if (value['frames'] === void 0) {
      frames = value['frames'];
    }
