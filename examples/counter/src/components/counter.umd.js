"use strict";
function _$tplCounter(state, children) {
    ...
    setTxt_2 = function (state) { return (state.count); };
    bindClassStrong = function (state) { return (_$bc(state.negative())).trim(); };
    clickEvent = function (state, $event, $el) {
        state.increment();
    };
    clickEvent_1 = function (state, $event, $el) {
        state.decrement();
    };
    return {
        $create: function () {
...
function Counter(attrs) {
    _$CompCtr.call(this, attrs, _$tplCounter, { model: { count: 0, increment: function () { this.count = this.count + 1; }, decrement: function () { this.count = this.count - 1; }, negative: function () { return { 'negative': this.count < 0 }; } } });
}
